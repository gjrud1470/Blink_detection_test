package com.example.blink_detection_test
...
class CamService: Service() {
    ...
    private var leftEyeOpenProbability : Float = FirebaseVisionFace.UNCOMPUTED_PROBABILITY
    private var rightEyeOpenProbability : Float = FirebaseVisionFace.UNCOMPUTED_PROBABILITY
    //private var leftopenRatio : Double = 1.toDouble()
    private var blinkCount : Int = 0

    // Face Detector
    val eyedetectOpt = FirebaseVisionFaceDetectorOptions.Builder()
        .setLandmarkMode(FirebaseVisionFaceDetectorOptions.ALL_LANDMARKS)
        .setClassificationMode(FirebaseVisionFaceDetectorOptions.ALL_CLASSIFICATIONS)
        .build()
    /*
    private var detector = FaceDetector.Builder( applicationContext )
    ...
    */
    ...
    private val imageListener = ImageReader.OnImageAvailableListener { reader ->
        ...
        if (image != null) {
            val faceimage = FirebaseVisionImage.fromMediaImage(image, 0)
            val detector = FirebaseVision.getInstance()
                .getVisionFaceDetector(eyedetectOpt)
            val result = detector.detectInImage(faceimage)
                .addOnSuccessListener { faces ->
                    if (faces.size != 0) {
                        //Log.wtf(TAG, "face recognized size of faces: ${faces.size}")
                        val face = faces[0]
                        val currentLeftEyeOpenProbability = face.leftEyeOpenProbability
                        val currentRightEyeOpenProbability = face.rightEyeOpenProbability
                        var blinked = false

                        if (currentLeftEyeOpenProbability == FirebaseVisionFace.UNCOMPUTED_PROBABILITY
                            || currentRightEyeOpenProbability == FirebaseVisionFace.UNCOMPUTED_PROBABILITY
                        ) {
                        } else {
                            if (leftEyeOpenProbability > 0.85 || rightEyeOpenProbability > 0.85) {
                                if (currentLeftEyeOpenProbability < 0.6 || currentRightEyeOpenProbability < 0.6) {
                                    blinked = true
                                }
                        ...
                        if (blinked) {
                            blinkCount++
                            Log.wtf("blinked!", "blink count is ${blinkCount}")
                        }
                    }
                }
                .addOnFailureListener { e ->
                    Log.wtf("Face Detector", "failed to detect face")
                }
        }
